...
import { yahooFinanceImporter } from './services/yahooFinanceImporter';
...
export async function registerRoutes(app: Express): Promise<Server> {
  /**
   * GET /api/crossovers/:date - Retrieves crossover signals for a specific date
   * Path parameters:
   * - date: Filter by specific date (YYYY-MM-DD)
   */
  app.get("/api/crossovers/:date", async (req, res) => {
    try {
      const { date } = req.params;
      const { signalType } = req.query;

      // Get crossover signals from storage with filters
  ...
  /**
   * POST /api/import-yahoo-history - Imports historical data from Yahoo Finance
   * Fetches 60+ days of historical stock price data for all tracked stocks
   * Runs asynchronously in the background
   * Body: { days: number } (optional, defaults to 60)
   */
  app.post("/api/import-yahoo-history", async (req, res) => {
    try {
      const schema = z.object({
      ...
      // Start Yahoo Finance import in background (non-blocking)
      yahooFinanceImporter.importHistoricalDataFromYahoo(days)
        .catch((error: Error) => console.error("Yahoo Finance import error:", error));

      res.json({ 
        success: true, 
        message: `Yahoo Finance historical data import started (${days} days)` 
      });
    } catch (error) {
      console.error("Error starting Yahoo Finance import:", error);
      res.status(500).json({ 
        error: "Failed to start Yahoo Finance import",
        details: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });
  ...
  /**
   * POST /api/calculate-crossovers - Calculate crossovers for existing historical data
   * Processes all dates in the database and detects MA crossover signals
   */
  app.post("/api/calculate-crossovers", async (req, res) => {
    try {
      const { maService } = await import('./services/movingAverage');
      ...
      for (const date of allDates) {
        ...
        if (crossovers.length > 0) {
          // Convert to InsertCrossoverSignal format
      ...
      res.json({ 
        success: true, 
        message: `Calculated crossovers for ${allDates.length} dates, found ${crossoverCount} signals`
      });
    } catch (error) {
      console.error("Error calculating crossovers:", error);
      res.status(500).json({ 
        error: "Failed to calculate crossovers",
        details: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });
    ...
    try {
      ...
      const candidates = topStocks.map(price => {
        ...
        return {
          ...
          volumeValue: parseFloat((parseInt(price.volumeValue) / 100000000).toFixed(2)),
